...
window.onload = function () {
  if (
    ...
    let color = (header.style.background = "rgba(" + "226, 149, 120, 1)");
  } else {
    window.addEventListener("scroll", (e) => {
      ...
      let opOne = 1;
      let idx = parseInt((pageYOffset / 1000) * 10);

      if (pageYOffset <= 1000) {
        let color = (header.style.background =
          "rgba(" + "226, 149, 120," + " ." + idx + ")");
      }
    });
  }

  const titleRadio = document.querySelector(
    "#product > div.form-group.col-sm-6.selectpicker-wrapper.required.option-group > label"
  );

  if (titleRadio === "цвет" || titleRadio === "Цвет") {
  }
  ...
  rad.forEach((el) => {
    let label = el.querySelector(".custom-control-label");
    let lab = label.textContent.toLowerCase();
  ...
  document
    ...
    .addEventListener("click", () => {
      ...
      dropDown.forEach((el) => {
        let label = el.querySelector(".dropdown-item");
        let lab = label.textContent.toLowerCase();
  ...
  search.addEventListener("mouseout", (e) => {
    e.target.closest("#search").style.cssText =
      "max-width: 24px; border-bottom: none;";
  });

  const clone = document
    .querySelector(
      "#undefined-sticky-wrapper > header > div.navigation-wrapper > div > nav"
    )
    .cloneNode(true);
  ...
  const menuHasChildrenCopy = Object.assign([], menuHasChildren);
  ...
  function createContainer(clss, list, id) {
    ...
    container.classList.add(clss);
    containerInner.classList.add("container-fluid");

    container.setAttribute("data-nav-part", id);

    header.append(container);
    container.append(containerInner);
    //            containerInner. insertAdjacentHTML('beforeend', img);
  }

  function addChildList(element) {
    element.forEach((el, idx) => {
      let attr = objCopy[idx].closest("li").getAttribute("data-nav-part");
  ...
  menuHasChildren.forEach((el, idx) => {
    el.addEventListener("click", (e) => {
      ...
      if (atr === data && !parent.matches(".show")) {
        //  if (e.target.textContent.trim() === titleAndImg[idx].name) {
        //     let image = headerSub[idx].querySelector('img');
        //    image.setAttribute('src', titleAndImg[idx].image);
        //}
        headerSub[idx].classList.add("show");
        document
          .querySelector("#undefined-sticky-wrapper > header")
  ...
  // VIDEO
  //     const bannerImg = document.querySelector("#banner0 > div > div > div > div > img");
